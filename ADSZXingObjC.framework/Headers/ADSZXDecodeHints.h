...
@protocol ADSZXResultPointCallback;
@class ADSZXIntArray;
 ...
 */
@interface ADSZXDecodeHints : NSObject <NSCopying>
...
/**
 * The caller needs to be notified via callback when a possible ADSZXResultPoint
 * is found.
 */
@property (nonatomic, strong) id <ADSZXResultPointCallback> resultPointCallback;
 ...
 * Maps to an ADSZXIntArray of the allowed extension lengths, for example [2], [5], or [2, 5].
 * If it is optional to have an extension, do not set this hint. If this is set,
 * and a UPC or EAN barcode is found but an extension is not, then no result will be returned
 * at all.
 */
@property (nonatomic, strong) ADSZXIntArray *allowedEANExtensions;

/**
 * Image is known to be of one of a few possible formats.
 */
- (void)addPossibleFormat:(ADSZXBarcodeFormat)format;
- (BOOL)containsFormat:(ADSZXBarcodeFormat)format;
- (int)numberOfPossibleFormats;
- (void)removePossibleFormat:(ADSZXBarcodeFormat)format;
