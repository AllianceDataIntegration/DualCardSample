...
typedef NS_ENUM(NSInteger, ADSFSCalendarCellState) {
    ADSFSCalendarCellStateNormal      = 0,
    ADSFSCalendarCellStateSelected    = 1,
    ADSFSCalendarCellStatePlaceholder = 1 << 1,
    ADSFSCalendarCellStateDisabled    = 1 << 2,
    ADSFSCalendarCellStateToday       = 1 << 3,
    ADSFSCalendarCellStateWeekend     = 1 << 4,
    ADSFSCalendarCellStateTodaySelected = ADSFSCalendarCellStateToday|ADSFSCalendarCellStateSelected
};

typedef NS_ENUM(NSUInteger, ADSFSCalendarCellShape) {
    ADSFSCalendarCellShapeCircle    = 0,
    ADSFSCalendarCellShapeRectangle = 1
};

typedef NS_OPTIONS(NSUInteger, ADSFSCalendarCaseOptions) {
    ADSFSCalendarCaseOptionsHeaderUsesDefaultCase      = 0,
    ADSFSCalendarCaseOptionsHeaderUsesUpperCase        = 1,
    
    ADSFSCalendarCaseOptionsWeekdayUsesDefaultCase     = 0 << 4,
    ADSFSCalendarCaseOptionsWeekdayUsesUpperCase       = 1 << 4,
    ADSFSCalendarCaseOptionsWeekdayUsesSingleUpperCase = 2 << 4,
};

/**
 * ADSFSCalendarAppearance determines the fonts and colors of components in the calendar.
 *
 * @see ADSFSCalendarDelegateAppearance
 */
@interface ADSFSCalendarAppearance : NSObject
 ...
 *
 * @see ADSFSCalendarCellShape
 */
@property (assign, nonatomic) ADSFSCalendarCellShape cellShape;

/**
 * The case options manage the case of month label and weekday symbols.
 *
 * @see ADSFSCalendarCaseOptions
 */
@property (assign, nonatomic) ADSFSCalendarCaseOptions caseOptions;
 ...
 */
@interface ADSFSCalendarAppearance (Deprecated)

@property (assign, nonatomic) ADSFSCalendarCellStyle cellStyle ADSFSCalendarDeprecated('cellShape');
@property (assign, nonatomic) BOOL useVeryShortWeekdaySymbols ADSFSCalendarDeprecated('caseOptions');
@property (assign, nonatomic) BOOL autoAdjustTitleSize ADSFSCalendarDeprecated('adjustFontSizeToFitContentSize');
@property (assign, nonatomic) BOOL adjustsFontSizeToFitCellSize ADSFSCalendarDeprecated('adjustFontSizeToFitContentSize');

@property (assign, nonatomic) CGFloat titleTextSize ADSFSCalendarDeprecated('titleFont');
@property (assign, nonatomic) CGFloat subtitleTextSize ADSFSCalendarDeprecated('subtitleFont');
@property (assign, nonatomic) CGFloat weekdayTextSize ADSFSCalendarDeprecated('weekdayFont');
@property (assign, nonatomic) CGFloat headerTitleTextSize ADSFSCalendarDeprecated('headerTitleFont');
