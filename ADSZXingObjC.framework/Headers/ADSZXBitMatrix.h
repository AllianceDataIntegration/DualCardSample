...
@class ADSZXBitArray, ADSZXIntArray;
 ...
 */
@interface ADSZXBitMatrix : NSObject <NSCopying>
...
+ (ADSZXBitMatrix *)parse:(NSString *)stringRepresentation
...
/**
 * Exclusive-or (XOR): Flip the bit in this ADSZXBitMatrix if the corresponding
 * mask bit is set.
 *
 * @param mask XOR mask
 */
- (void)xor:(ADSZXBitMatrix *)mask;
...
/**
 * A fast method to retrieve one row of data from the matrix as a ADSZXBitArray.
 *
 ...
 *         your own row
 */
- (ADSZXBitArray *)rowAtY:(int)y row:(ADSZXBitArray *)row;

/**
 * @param y row to set
 * @param row ADSZXBitArray to copy from
 */
- (void)setRowAtY:(int)y row:(ADSZXBitArray *)row;

/**
 * Modifies this ADSZXBitMatrix to represent the same but rotated 180 degrees
 */
- (void)rotate180;
 ...
 */
- (ADSZXIntArray *)enclosingRectangle;
 ...
 */
- (ADSZXIntArray *)topLeftOnBit;

- (ADSZXIntArray *)bottomRightOnBit;
